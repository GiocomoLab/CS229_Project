function [smooth_firing_rate,firing_rate] = calculateFiringRate(idx,posx,dt,p,binedges,smoothWindow)
...
% bins.
    ...
    if isempty(idx)
        ...
        smooth_firing_rate = zeros(1,nbins);
    else        
        ...
        if noTaper
            smooth_firing_rate = conv([repmat(firing_rate(1),floor(smoothWindow/2),1); firing_rate; ...
                repmat(firing_rate(end),floor(smoothWindow/2),1)],gauss_filter,'valid');
        else
            smooth_firing_rate = conv(smooth_firing_rate,gauss_filter,'same');
        end
