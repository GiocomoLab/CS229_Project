function [inField, outField] = defineFields(fr, fr_dist, cent, idx_spike, posx,p)
...
% 5/31/17 MGC
    ...
    frp = nan(3,nbins);
    frp(1,:) = prctile(fr_dist,p.InFieldThreshold);
    frp(2,:) = prctile(fr_dist,p.FieldExtensionThreshold);
    frp(3,:) = prctile(fr_dist,p.OutFieldThreshold);
